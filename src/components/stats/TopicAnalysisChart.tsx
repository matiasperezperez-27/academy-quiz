...
export const TopicAnalysisChart = ({
  topics
}: TopicAnalysisChartProps) => {
  if (!topics || topics.length === 0) {
    return <Card className="backdrop-blur-sm bg-white/95 dark:bg-gray-800/95 border border-gray-200 dark:border-gray-700 shadow-xl shadow-gray-500/10">
  ...
  return <Card className="backdrop-blur-sm bg-white/95 dark:bg-gray-800/95 border border-gray-200 dark:border-gray-700 shadow-xl shadow-gray-500/10">
      ...
      <CardContent className="p-6">
        <div className="mb-6 p-4 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl border border-blue-200/30 dark:border-blue-700/30 px-0">
          <ResponsiveContainer width="100%" height={320}>
            ...
            bottom: 80
              ...
              <XAxis dataKey="name" angle={-45} textAnchor="end" height={100} className="text-xs" tick={{
              ...
              <Bar dataKey="accuracy" fill="url(#colorGradient)" radius={[6, 6, 0, 0]} strokeWidth={1} stroke="rgba(59, 130, 246, 0.3)" />
              <defs>
          ...
          <div className="space-y-3">
            {topics.map(topic => <div key={topic.topicId} className="group flex items-center justify-between p-5 bg-gradient-to-r from-gray-50/80 to-blue-50/30 dark:from-gray-800/50 dark:to-blue-900/10 rounded-xl hover:shadow-lg hover:scale-[1.02] transition-all duration-300 border border-gray-200/50 dark:border-gray-700/50 hover:border-blue-300/50 dark:hover:border-blue-600/50">
                ...
                <div className="flex items-center gap-6 flex-shrink-0">
                  <div className="text-right">
                    <div className="flex items-center gap-2 justify-end mb-1">
                      ...
                      <div className="flex items-center justify-center w-6 h-6 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600">
                        {topic.trend === 'up' && <TrendingUp className="h-3 w-3 text-green-600 dark:text-green-400" />}
                        {topic.trend === 'down' && <TrendingDown className="h-3 w-3 text-red-600 dark:text-red-400" />}
                        {topic.trend === 'stable' && <Minus className="h-3 w-3 text-yellow-600 dark:text-yellow-400" />}
                      </div>
                    </div>
                  ...
                  <Badge variant={topic.accuracy >= 80 ? "default" : topic.accuracy >= 60 ? "secondary" : "destructive"} className={`min-w-[100px] justify-center font-semibold shadow-sm ${topic.accuracy >= 80 ? "bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white border-0" : topic.accuracy >= 60 ? "bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white border-0" : "bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white border-0"}`}>
                    {topic.accuracy >= 80 ? "🏆 Dominado" : topic.accuracy >= 60 ? "📈 En progreso" : "📚 Necesita práctica"}
                  </Badge>
                </div>
              </div>)}
          </div>
