...
export const StreakCounter = ({
  ...
  return <Card className="bg-gradient-to-r from-orange-50 to-red-50 border-orange-200">
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Flame className={cn("h-6 w-6", getStreakColor())} />
              <h2 className="text-xl font-bold text-stone-950">Racha de Estudio</h2>
            </div>
        ...
        {streak > 0 && <div className="mt-4 flex gap-1">
            {Array.from({
          length: Math.min(streak, 30)
        }, (_, i) => <div key={i} className={cn("h-2 flex-1 rounded-full", i < 7 ? "bg-orange-400" : i < 14 ? "bg-orange-500" : i < 21 ? "bg-orange-600" : "bg-red-600")} style={{
          maxWidth: "20px"
        }} />)}
          </div>}
      </CardContent>
