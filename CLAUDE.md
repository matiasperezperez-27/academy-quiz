...
**Auto-resizing textareas**: shadcn `<Textarea>` has `min-h-[80px]` baked into its base class, which prevents the `scrollHeight` collapse trick from working. For textareas that must start at 1 line and grow to content (e.g. option fields in `PreguntaFormDialog`), use a plain `<textarea>` HTML element with `rows={1}` (no inline `style` height — React would re-apply it on every render and override the measured height) + `resize-none overflow-hidden`. Resize logic: (1) `useEffect` + `requestAnimationFrame` depending on `[open, ...values]` — `useLayoutEffect` fires before Radix Dialog portals finish layout so `scrollHeight` is unreliable; `rAF` defers to after the first paint when dimensions are final; (2) inline resize in `onChange` for immediate user feedback. Both use `el.style.height = 'auto'; el.style.height = el.scrollHeight + 'px'`. The component must be **conditionally mounted** (`{flag && <Dialog open={flag}.../>}`) so it always mounts with `open=true` and the `useEffect` fires with refs attached.
